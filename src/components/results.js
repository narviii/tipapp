function StringRes({ text, amount,isLarge }) {
    ...
    //console.log(isLarge)
    if (isLarge != true) {
         res = <p className="md:text-5xl text-4xl text-right "> {amount} </p>
    } else {
         res = <p className="md:text-2xl text-xl text-right "> {amount} </p>
    }
    return (
...
function Results({tipAmount,total,reset}) {
    ...
    return (
        <div className="md:w-1/2 w-full h-full bg-green-700 rounded-lg p-6">
